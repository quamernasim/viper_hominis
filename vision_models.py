...
class HominisModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        with open(config.hominis.qa_prompt) as f:
            self.qa_prompt = f.read().strip()
        with open(config.hominis.guess_prompt) as f:
            self.guess_prompt = f.read().strip()
        self.temperature = config.hominis.temperature
        self.n_votes = config.hominis.n_votes
        self.model = config.hominis.model
    ...
    def process_guesses_fn(self, prompt):
        ...
        response = self.query_hominis(prompt, model=self.model, max_tokens=5, logprobs=1, stream=False,
                                   stop=["\n", "<|endoftext|>"])
        return response
    ...
    def get_qa(self, prompts, prompt_base: str = None) -> list[str]:
        ...
        final_prompt = prompt_base.format(prompts)
        print(final_prompt)
        response = self.get_qa_fn(final_prompt)
        return response
    ...
    def get_general(self, prompts) -> list[str]:
        response = self.query_hominis(prompts, model=self.model, max_tokens=256, top_p=1, frequency_penalty=0,
                                   presence_penalty=0)
        return response
    
    def call_hominis_api_model(self, prompt):
        body = {
            "model": config.hominiscode.model,
            "prompt": prompt,
            "stream": False,
            "options": {
                "num_predict": config.hominiscode.num_predict
            },
            "seed": 420,
            "system": 'You are an Intelligent Bot which is tasked at answering questions and providing information concerning the given prompt.'
        }
        response = requests.post(config.hominiscode.BASE_URL, json=body)
        resp = response.json()['response']
        return resp

    def query_hominis(self, prompt, model="hominis-instruct", max_tokens=16, logprobs=None, stream=False,
                   stop=None, top_p=1, frequency_penalty=0, presence_penalty=0):
        response = self.call_hominis_api_model(prompt)
        return response

    def forward(self, prompt, process_name):

        if process_name == 'hominis_qa':
            ...
            if isinstance(prompt, tuple) or isinstance(prompt, list):
                ...
                prompt = question.format(context)

        if process_name == 'hominis_qa':
            response = self.get_qa(prompt)
        elif process_name == 'hominis_guess':
    ...
    @classmethod
    def list_processes(cls):
        return ['hominis_' + n for n in ['qa', 'guess', 'general']]


def call_hominis_api(prompt):
    body = {
        "model": config.hominiscode.model,
        "prompt": prompt,
        "stream": False,
        ...
        "seed": 420,
        "system": config.hominiscode.system_prompt
    }
    response = requests.post(config.hominiscode.BASE_URL, json=body)
    resp = response.json()['response']
    return resp
...
def hominiscode_helper(extended_prompt):
    # assert 0 <= config.hominiscode.temperature <= 1
    # assert 1 <= config.hominiscode.best_of <= 20
    resp = call_hominis_api(extended_prompt)

    return resp


class HominisCodeModel(BaseModel):
    name = 'hominiscode'
    requires_gpu = False
    max_batch_size = 5
    ...
    def __init__(self, gpu_number=0):
        super().__init__(gpu_number=gpu_number)
        with open(config.hominiscode.prompt) as f:
    ...
    def process_code(self, code):
        ...
        lines = code.split('\n')
        # Find the indices for the start of the function and the last return
        start_index = next(i for i, line in enumerate(lines) if line.strip().startswith("def "))
        end_index = max(i for i, line in enumerate(lines) if line.strip().startswith("return"))
        # Extract the code block
        code = '\n'.join(lines[start_index:end_index+1])
        code = code.replace('execute_command(image)', 'execute_command(image, my_fig, time_wait_between_lines, syntax)')
        return code
    ...
    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt=None, extra_context=None):
        ...
        result = self.forward_(extended_prompt[0])
        result = self.process_code(result)
        return result

    def forward_(self, extended_prompt):
        try:
            response = hominiscode_helper(extended_prompt)
        except Exception as e:
            # Some other error like an internal OpenAI error
            print("Retrying HominisCode")
            print(e)
